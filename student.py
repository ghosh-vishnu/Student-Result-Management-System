...
class StudentClass:
    def __init__(self, root) :
        ...
        # ================================================title==========================================================
        title = Label(self.root, text="Manage Student Details", font=("goudy old", 20, "bold"), bg="#0000ff", fg="white").place(x=10, y=15, width=1180, height=35)
        # ==================================================Variable=====================================================
    ...
    def add(self):
        ...
        try:
            if self.var_roll.get()=='':
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror('Error', 'Roll Number is Already Present', parent=self.root)
                else:
                    cur.execute('insert into student(roll,name,email,gender,contact,admission,course,state,city,pin,address) values(?,?,?,?,?,?,?,?,?,?,?)',(
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_email.get(),
                    self.var_gender.get(),
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row == None :
                    ...
                    cur.execute("update student set name=?,email=?,gender=?,contact=?,admission=?,course=?,state=?,city=?,pin=?,address=? where roll=?",(
                    self.var_name.get(),
                    self.var_email.get(),
                    self.var_gender.get(),
                    # self.var_dob.get(),
